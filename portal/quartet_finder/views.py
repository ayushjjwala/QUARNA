...
import os
import urllib2
import linear_nomenclature
import star_nomenclature
import cyclic_nomenclature
import semicyclic_nomenclature
# import linear_test
# import star_test
# import cyclic_test
	...
	if request.method=='POST':
		...
		if(pdb_files==None) and (pdb_name==''):
			...
			if val==1: #download was not successfull
				print "PDB File: "+pdb_name+" download was not successfull"
...
def download_pdb(name):
	try:
		print "downloading PDB File from web:",name
...
def handle_upload(f):
	print "Downdling the PDB File uploaded:",str(f)
...
def run_bpfind(pdb):
	print "Running BPFIND on the PDB File:",pdb
	folder = pdb.split('.')[0]
...
def run_quadScripts(pdb):
	os.chdir("..")
	os.system("pwd")
	print "Extracting quartets from ",pdb
	...
	print "Output files is ready:",pdb
...
def specific_residue(request):
	if request.method=="POST":
		...
		if res[0] not in check_res or res[1] not in check_res or res[2] not in check_res or res[3] not in check_res:
			...
			script_path = "scripts/specific_residue/" + topology + ".py" #run <topology>.py to find the specific mentioned residues 
			...
			if os.stat(file_path).st_size == 0: #checking for empty file.
				print "file is empty."
...
def linear(request):
	if request.method == 'POST':
		...
		script_path = "scripts/linear/" + pos + ".py" #run <pos>.py to extract the specific linear quartets
		os.system("python " + script_path + " " + args + " > " + file_path)
		if os.stat(file_path).st_size == 0: #checking for empty file.
			print "file is empty."
			file_empty = "There are no results to show."
...
def star(request):
	if request.method=='POST':
		...
		args = temp[0] + ' ' + temp[1] + ' ' + orient + ' ' + dataset
		script_path = "scripts/star/bp.py" #run bp.py to extract the specific star quartets
		os.system("python " + script_path + " " + args + " > " + file_path)
		if os.stat(file_path).st_size == 0: #checking for empty file.
			print "file is empty."
			file_empty = "There are no results to show."
...
#Find the cyclic quartets with specific base pairs
def cyclic(request):
	if request.method=='POST':
		b1 = request.POST['b1']
		b2 = request.POST['b2']
		...
		args = temp[0] + ' ' + temp[1] + ' ' + orient + ' ' + dataset
		script_path = "scripts/cyclic/bp.py" #run bp.py to extract specific cyclic quartets
		os.system("python " + script_path + " " + args + " > " + file_path)
		if os.stat(file_path).st_size == 0: #checking for empty file.
			print "file is empty."
			file_empty = "There are no results to show."
...
def semi_cyclic(request):
	if request.method=='POST':
		...
		args = temp[0] + ' ' + temp[1] + ' ' + orient + ' ' + dataset
		script_path = "scripts/semi_cyclic/bp.py" #run bp.py to extract specific semi-cyclic quartets
		os.system("python " + script_path + " " + args + " > " + file_path)
		if os.stat(file_path).st_size == 0: #checking for empty file.
			print "file is empty."
			file_empty = "There are no results to show."
...
#Using JSMOL applet to display all the quartets in a PDB file
def jsmol(request,pdb_name):
	...
	print "Running JSMOL applet to display the PDB File:",pdb_name
...
#Using JSMOL applet to display the linear quartets with specific base pair in different PDB files
def view_linear(request,file_path,file_name):
	...
	dataset = check_xray(pdb_name)
	print "Viewing linear quartets in the file:",file_name + "....."
...
#Using JSMOL applet to display the star quartets with specific base pair in different PDB files
def view_star(request,file_path,file_name):
	...
	dataset = check_xray(pdb_name)
	print "Viewing star quartets in the file:",file_name + "....."
...
#Using JSMOL applet to display the cyclic quartets with specific base pair in different PDB files
def view_cyclic(request,file_path,file_name):
	...
	dataset = check_xray(pdb_name)
	print "Viewing cyclic quartets in the file:",file_name + "....."
...
#Using JSMOL applet to display the semi-cyclic quartets with specific base pair in different PDB files
def view_semicyclic(request,file_path,file_name):
	...
	dataset = check_xray(pdb_name)
	print "Viewing semi-cyclic quartets in the file:",file_name + "....."
...
#Extract the linear quartets from the output file
...
#Extract the star quartets from the output file
...
#Extract the cyclic quartets from the output file
...
#Extract the semi-cyclic quartets from the output file
