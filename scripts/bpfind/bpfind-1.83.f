...
C  
C  Das, S. Mukherjee, A. Mitra and D. Bhattacharyya (2006) Non-Canonical Base 
...
C
     ...
     2         chohb,hetatm,sugarbp,nforce,oligo
       real cutoff,energy,energylc,calenrg,varenergy,cutang,occ
       real anglinfo,varangl,yangle,anglinfb,bfctr ,bf,xa,ya,za,ocp
	logical raretauto
       CHARACTER*4 NAMED,atmnam,ATOM
       CHARACTER*3 NUCL,resn,resd,locf(30),adevar(200),guavar(200),
     1   cytvar(200),uravar(200)
       Character*3 feature,bpinfo,bs,bptype,bpinfb,bptypeb,resid,ans(2)
       character*1 type,tpinfo,tp,tpinfb,pos(100000),tmppos,tpod(30),
     1 posins,CHAIND,chainid,pchaind,molid,chan,loct(30),pcd,strseq
       character*2 bptp(30),locb(30)
       Character*80 filenm,nmpass,title,LINE,ttl(50),aline
	character*8 aba(10000),aca(10000),baa(10000),bca(10000)
        integer iaa(10000),iba(10000),ica(10000),ida(10000),iloop(10000)
       character*50 val1(20),valp,valv
	real invdegree
       COMMON /HBQUA/BASE(21,100000),energy(20,100000),yangle(20,100000)
       COMMON /HBQTYP/TYPE(20,100000),FEATURE(20,100000)
       COMMON /NAMES/NAMED(9000000),RESD(9000000)
       COMMON /CHAINS/CHAIND(9000000)
       COMMON /ALPHA/XD(9000000),YD(9000000),ZD(9000000),bfctr(100000)
       COMMON /BETA/IRESD(9000000),ISTS(1000000),IENS(1000000),nres,
     1              nforce
       COMMON /GAMA/ATGC(100000)
       common /options/cutang,cuteng,cutoff
       common /residue/resid(9000000)
       common /occur/occ(9000000)
       common /num/presd(100000),prd(100000)
       common /cum/pchaind(100000),pcd(100000),molid
       common /str/strseq(100000)
	data ans/'No ','Yes'/
        open(unit=14,file='AdeVariants.name')
	i=1
	do while (i.le.200)
...
115	navar=i-1
	close(unit=14)
        open(unit=14,file='GuaVariants.name')
	i=1
	do while(i.le.200)
...
114	ngvar=i-1
	close(unit=14)
        open(unit=14,file='CytVariants.name')
	i=1
	do while(i.le.200)
...
117	ncvar=i-1
	close(unit=14)
        open(unit=14,file='UraVariants.name')
	i=1
	do while(i.le.200)
...
118	continue
	nuvar=i-1
 15	format(a3)
c        DATA NUCL/ '  A','ADE','1MA','MIA',' +A','AMP','AMO','12A',
c     1 'AET','PSD','AVC','APC','GOM','MAD','A23','ATP','2MA','A2M',
c     1 'T6A','RIA','6MZ','6IA',' DA','ADP','5AA','PR5','2AD','3DA',
c     1 'ANZ',
c                                                   ! 29 Ade variants
c     2 '  G','GUA','2MG','OMG','G7M','GNP','QUO','7MG','1MG','5CG',
c     2 ' +G','GDP','M2G','  I','GMP','GTP',' DG','PGP',' YG','YYG',
c     2 '2PR','XUG',
c                                                   ! 22 Gua variants
c     3 '  C','CYT','5MC',' +C','OMC','S4C','CB2','5IC','CCC','1SC',
c     3 ' DC','CBV','DCZ','CSL','CBR',
c                                                   ! 15 Cyt variants
c     4 '  T','THY','  U','URA','H2U','5MU','2MU','4SU','FMU','CMO',
c     4 'OMU','70U',' +U','DHU','UR3',' RT','5BU',' DT','S4U','MTU',
c     4 'MNU','UMS',' IU','UD5','PYO','SUR','SSU',
c                                                   ! 27 Ura variants
c     5 'N6G',
c     6 'PSU','FHU'/
	...
	narg=iargc()
	  ...
	  write(6,9999)
	call showoptn
9999	format(/'Invalid Input'/'  
     1[USAGE]: BPFIND -[options] filename'/'See README.txt for HELP')
	  stop
	endif
        do i=1,narg
	...
	write(*,*) 'Options used are:'
	write(*,*) '        cutoff Distance^2=',cutoff
	write(*,*) '        CutAngle=',cutang
	write(*,*) '        CutEnergy=',cuteng
	write(*,*) '        CH...O HB: ',ans(chohb+1)
	write(*,*) '        Sugar HB: ',ans(sugarbp+1)
	write(*,*) '        Choise of HETATM entries: ',ans(hetatm+1)
	write(*,*) '        Option as Oligonucleotide: ',ans(oligo+1)
	write(*,*) '        Mol-ID: ',molid
	kkforce = nforce + 2
	if(kkforce.gt.2) kkforce=kkforce-2
      	write(*,*) '        Pairing restriction: ',ans(kkforce)
        title=filenm
        nnf=index(filenm,'.')
        open(unit=4,file=filenm,status='old',err=211)
        nmpass=filenm
	...
	do it=1,kkti
	  ...
	  write(9,5) ttl(it)
	enddo
	write(52,6) filenm(1:index(filenm,'.'))
        write(52,73)
        write(52,74)hbdst,cutang,cuteng
        do 2 n=1,900000
          read(4,5,end=99)line
	  if(nmr.eq.1.and.line(1:6).eq.'ENDMDL') goto 99
          if((line(1:4).eq.'ATOM'.or.(hetatm.eq.1.and.line(1:6).eq.
     1 'HETATM')).and.((molid.eq.' ').or.(line(22:22).eq.molid)))then
            read(line,4) atmnam,resn,chan,nores,posins,xa,ya,za,ocp,bf
	    if(atmnam(4:4).eq.'A') atmnam(4:4)=' '
     ...
     1.ne.'QUO'))then
               write(*,337)nores,resn,chan
               write(52,337)nores,resn,chan
               endif

337            format('MODRES',3X,I5,2X,A3,2X,A1)
338            format('=========='/'Number of POORLY defined atoms:',i5
     1  /'==========')
	       if(nfound.eq.1)then
	         named(l)=atmnam
	         resd(l)=resn
	         ...
	         occ(l)=ocp
	         bfctr(l)=bf
                 if(lresno.ne.iresd(l).or.tmppos.ne.pos(l)) then
                   nres=nres+1
                   ists(nres)=l
                   lresno=iresd(l)
	           tmppos=pos(l)
...
c              write(2,110)l,named(l),resd(l),chaind(l),i,
c     1                      xd(l),yd(l),zd(l),occ(l),bfctr(l)
c           end do
c         end do
110     format('ATOM',1X,I6,2X,A4,A3,1X,A1,I4,4X,3F8.3,2x,f4.2,F6.2)
c        iens(nres)=l
        natd = l-1
	call cleanpdb(filenm)
	write(52,77)
        write(52,60)
        write(52,61)
        write(52,62)
...
c       ADE-ADE (wwC) HB betn. N1A-N6A & C2A-N1A
        call findpair(i,1,'C5  ','N1  ','N6  ','N7  ',1,'N3  ','C2  ',
     1  'N1  ','C4  ','C','w:w','w:w',0)

c       ADE-N6G (wwC) HB betn. N1A-N6A & C2A-N1A
        call findpair(i,1,'C5  ','N1  ','N6  ','N7  ',5,'N3  ','C2  ',
     1  'N1  ','C4  ','C','w:w','w:w',0)
...
c       ADE-ADE (wsC) HB betn. N1A-C2A & C2A-N3A
        call findpair(i,1,'C5  ','N1  ','C2  ','C6  ',1,'C4  ','C2  ',
     1  'N3  ','C5  ','C','w:s','s:w',0)
...
c       ADE-ADE (ssT) HB betn. N3A-C2A & C2A-N3A
        call findpair(i,1,'C5  ','N3  ','C2  ','C6  ',1,'C6  ','C2  ',
     1  'N3  ','C5  ','T','s:s','s:s',0)
...
c       ADE-GUA (swC) HB betn. C2A-O6G & N3A-N1G
        call findpair(i,1,'C6  ','C2  ','O6  ','N3  ',2,'C5  ','N3  ',
     1  'N1  ','C4  ','C','s:w','w:s',0)
...
c       ADE-URA (swC) HB betn. N3A-N3U & C2A-O4U
        call findpair(i,1,'C5  ','N3  ','N3  ','C2  ',4,'C6  ','C2  ',
     1  'O4  ','C5  ','C','s:w','w:s',0)
...
c       ADE-CYT (SST) HB betn. N3A-C1*C & C1*A-O2C
        call findpair(i,1,'C5  ','N3  ','C1* ','C6  ',3,'C8  ','C1* ',
     1  'O2  ','C5  ','T','S:S','S:S',1)
...
c       GUA-URA (SST) HB betn. N2G-C1*U & C1*G-O2U
        call findpair(i,4,'C5  ','O2  ','C1* ','C8  ',2,'C6  ','C1* ',
     1  'N2  ','N1  ','T','S:S','S:S',1)
          ...
          do j=1,base(1,i)
            ...
            if(tpod(j).eq.'N')then
              ...
              if(nc.gt.0)then 
                ...
                nn=j
c
c   Keeping only the main Hbonding edge if both canonical form and protonated
...
c     1 energy(1,i),yangle(1,i),' BASE(1,i)',base(1,i)
	...
	if(oligo.eq.1.and.(base(2,i).eq.(i-1).and.base(2,i-1).eq.i)) stop
 	   write(52,116) i,prd(i),resd(ists(i)),pcd(i),
     1 (base(j+1,i),prd(base(j+1,i)),resd(ists(base(j+1,i))),
...
c
     ...
     1        bptp(j),j=1,base(1,i))
          ...
          write(78,169) filenm(1:nnf)
169	format('>',a,' Secondary structural information')
        close(unit=4)
        close(unit=9)
        close(unit=52)
        close(unit=62)
...
62    format('----->Residue number as in PDB')
63    format('---------->Residue name')
64    format('--------------->Chain ID')
65    format('---------------->Serial number of the paired base')
66    format('--------------------->Residue number of the paired',
     1 ' base as in PDB')
67    format('-------------------------->Residue name of the paired',
     1 ' base')
68    format('------------------------------->Chain ID of the paired',
     1 ' base')
69    format('--------------------------------->Base pair type')
70    format('-------------------------------------->Base pair ',
     1  'indicator')
71    format('--------------------------------------->E-value ',
     1  'indicating base pair deformation')
72    format('------------------------------------------->Equivalent '
     1  ,'information for other pairs') 
...
116   format(I5,I5,1x,A3,1x,a1,4(I5,I5,1x,a3,1x,A1,1x,A3,A1,1x,a2,f5.2))
...
C      ----------------------------------------------------------######
     ...
     1              atp22,at13,atp13,at23,atp23
       ...
       character*1 config,type,chaind 
       COMMON /HBQUA/BASE(21,100000),energy(20,100000),yangle(20,100000)
       COMMON /HBQTYP/TYPE(20,100000),FEATURE(20,100000)
       COMMON /NAMES/NAMED(9000000),RESD(9000000)
       COMMON /CHAINS/CHAIND(9000000)
...
C
	subroutine cleanpdb(filenm)
     ...
     1              nforce
       ...
       CHARACTER*80 filenm
       CHARACTER*1 CHAIND,pchaind,pcd,fastaseq(100000),molid
	...
	do while (i.le.numresb)
	  ...
	  do k=ists(i),iens(i)
	    ...
	    do m=1,9
	      if(atomnm(m).eq.named(k)) natminr=natminr+1
	    enddo
	    if(named(k).eq.'C1* ') ncafnd=1
	  enddo
	  if(molid.ne.' '.and.molid.ne.pcd(m)) then
	  if(natminr.lt.6.or.ncafnd.eq.0) then
	    do m=i,nres-1
	      ists(m)=ists(m+1)
	      iens(m)=iens(m+1)
              prd(m)=prd(m+1)
...
76	format('HEADER   Cleaned number of residues: ',i6)
     ...
     1     .eq.nres) then
           ...
           do l=ists(i),iens(i)
              write(9,111)l,named(l),resd(l),chaind(l),i,
     1        xd(l),yd(l),zd(l),occ(l),bfctr(l)
          end do
        end do

	write(9,112)
111     format('ATOM',1X,I6,2X,A4,A3,1X,A1,I4,4X,3F8.3,2x,f4.2,F6.2)
112	format('END')
113     format(80a1)
114     format('>fromPDB:',a20,'forChainID:',a1)
	close(unit=9)
	close(unit=77)
	return
	end
...
C NNW : NO.OF HELICES CONTAINING AT LEST ONE NON-WATSON-CRICK BP
       subroutine rnahelix(nmpass,nres)
       DIMENSION IRES1(100000),IRES2(100000),IHLX(100000,2),nrstk(999,2)
     1           ,ndoub(999,2)
       DIMENSION IHELIX(1000,1000,2),M1(100000),ist1(10000),ist2(10000)
       INTEGER CHECK,Q
       CHARACTER*1 BASE1(100000),BASE2(100000),strseq,pchaind,
     1          pcd,molid
       CHARACTER*80 LINE,nmpass
       CHARACTER*5 BPAIR(100000),store(100000)
       common /str/strseq(100000)
       common /cum/pchaind(100000),pcd(100000),molid

       OPEN(UNIT=1,FILE=nmpass,STATUS='OLD',ERR=100)
       OPEN(UNIT=2,FILE='nwhelix.dat')
       OPEN(UNIT=3,FILE='wchelix.dat')
       open(unit=8,file='pshelix.dat')
       nn=index(nmpass,'.')
       nmpass(nn:nn+3)='.hlx'
       open(unit=9,file=nmpass)
...
C *********************************************************************       
       ...
       DO WHILE(K.EQ.0)
         ...
         IF(LINE(45:54).EQ.'Equivalent') THEN
           DO I=1,100000
           READ(1,15,END=10)IRES1(I),BASE1(I),IRES2(I),BASE2(I),BPAIR(I)
           ENDDO
         K=1
         ENDIF
       ENDDO
...
c      do i=1,nres
c        if(pchaind(i).ne.pchaind(i-1)) write(*,*)'Chain IDs change at',i
c      enddo
      write(78,168) (strseq(i),i=1,nres)
5     FORMAT(A80)
15    FORMAT(I5,6X,A3,2X,I5,6X,A3,2X,A5)
17	format(i5,1x,i5,a4,1x,a1)
